...
class MixupSharp(MixupModule):
    def __init__(
        self, alpha: float = 0.25, apply_max: bool = True, temperature: float = 0.3
    ):
    ...
    def forward(
        self, xa: Tensor, xb: Tensor, ya: Tensor, yb: Tensor
    ) -> (Tensor, Tensor):
        ...
        labels_mix = (
            ya * self._lambda ** self.temperature
            + yb * (1.0 - self._lambda) ** self.temperature
        )
