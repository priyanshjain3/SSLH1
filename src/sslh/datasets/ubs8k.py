...
class URBANSOUND8K(Dataset):
    ...
    def __init__(
        self, root: str = "data", folds: Iterable[int] = (), resample_sr: int = 22050
    ) -> None:
        """
        UBS8K dataset.
    ...
    def _load_metadata(self) -> Dict[str, list]:
        csv_path = os.path.join(
            self.root, self.ROOT_DNAME, "metadata", "UrbanSound8K.csv"
        )
...
class UBS8KDataset(URBANSOUND8K):
    ...
    def __getitem__(self, index: int) -> Tuple[Tensor, int]:
        if index in self._waveform_cache.keys():
            waveform = self._waveform_cache[index]
            target = self.meta["target"][index]
        else:
            waveform, target = super().__getitem__(index)
            if self.cached:
                self._waveform_cache[index] = waveform
